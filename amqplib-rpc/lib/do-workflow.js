...
/**
  ...
  * @param  {Buffer}   workflow  workflow descriptor
  ...
  */
module.exports = doWorkflow

function doWorkflow (connection, queueName, content, workflow, opts, cb) {
  ...
  var promise = connection.createChannel().then(function (channel) {
    ...
    var assertAndSendPromise = channel.assertQueue('', queueOpts).then(function (replyQueue) {
      ...
      var _sendOpts = put(sendOpts, {
        ...
        headers: {'X-WKF-INTERNAL-CORR-ID': corrId,
        'X-WKF-TERMINAL-ADDR': replyQueue.queue,
        'X-WKF-ERROR-ADDR': replyQueue.queue,
        'X-WKF-ROUTE-PROCESSED': '',
        'X-WKF-ROUTE-REMAIN' : workflow}
      })
      console.log('Send request message:', corrId, queueName, content, _sendOpts)
